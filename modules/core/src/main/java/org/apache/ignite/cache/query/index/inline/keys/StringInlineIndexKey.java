...
// TODO: git mv from StringInlineIndexColumn
public class StringInlineIndexKey extends NullableInlineIndexKey<String> {
    /** Default charset. */
    protected static final Charset CHARSET = StandardCharsets.UTF_8;

    // TODO: how to configure it? Looks like ignoreCase may be configured on moment of Search row creation.
    //      optimized is not in use?
...
//    private final boolean useOptimizedCompare;
    ...
    /** Constructor. */
    public StringInlineIndexKey(boolean compareIgnoreCase) {
        super(InlineIndexKeyType.STRING, (short) -1);  // -1 means variable length.

        this.compareIgnoreCase = compareIgnoreCase;
    }

    /** {@inheritDoc} */
    @Override protected int put0(long pageAddr, int off, String str, int maxSize) {
        short size;

        byte[] s = str.getBytes(CHARSET);
    ...
    /** {@inheritDoc} */
    @Override protected @Nullable String get0(long pageAddr, int off) {
        return new String(readBytes(pageAddr, off), CHARSET);
    }

    @Override protected int compare0(long pageAddr, int off, String s) {
